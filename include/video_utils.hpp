...
#endif
void draw_object(cv::Mat &img,
                    ai_service_msg::msg::IMGDetectionOutput &output,
                    const cv::Scalar &color = cv::Scalar{0,0,255}) {
  RCLCPP_DEBUG(rclcpp::get_logger("ImageUtils"),
              "x_offset: %f y_offset: %f width: %f "
              "height: %f",
              output.position_x, output.position_y, output.position_x_offset,
              output.position_y_offset);
  cv::rectangle(img, cv::Point(output.position_x, output.position_y),
                cv::Point((output.position_x + output.position_x_offset),
                          (output.position_y + output.position_y_offset)),
                color, 3);
  cv::putText(img, output.name,
              cv::Point2f(output.position_x, output.position_y - 10),
              cv::HersheyFonts::FONT_HERSHEY_SIMPLEX, 1.5, color, 2.5);
}
